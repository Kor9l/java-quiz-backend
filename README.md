...
2. **Render** — *New → Blueprint*, pointed at this repo. [`render.yaml`](render.yaml) declares the
   ...
   | Variable | Value |
   |---|---|
   | `DB_URL` | `jdbc:postgresql://<neon-host>/javaquiz?sslmode=require` |
   ...
   The three database variables were `SPRING_DATASOURCE_URL` / `_USERNAME` / `_PASSWORD` before
   the port. A service already running on those keeps booting — the app reads the old names as a
   fallback — but the blueprint now asks for the `DB_*` ones, so fill them in on the next sync
   and drop the old three afterwards.
