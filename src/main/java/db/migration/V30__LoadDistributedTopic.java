...
/**
 * Adds Microservices and distributed systems as a study topic: eight sections, their articles and
 * 48 quiz questions.
 *
 * <p>Follows the self-contained layout {@link V6__LoadSqlTopic} established — content under
 * {@code /content/distributed/} rather than in the shared files V2 reads, because V2 has
 ...
 *
 * <p>The career {@code level} added by {@code V7__levels} is carried by both sections and
 * questions. The column defaults to {@code MIDDLE}, so a missing level would load silently as
 * middle; {@code DistributedTopicContentTest} is what holds the content to an explicit value on
 * every row.
 */
public class V30__LoadDistributedTopic extends BaseJavaMigration {
