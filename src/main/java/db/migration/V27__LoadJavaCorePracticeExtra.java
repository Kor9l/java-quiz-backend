...
/**
 * Loads the second batch of Java Core exercises, the one that levels out the sections the
 * first batch left thin: exceptions, OOP basics, equals/hashCode and generics.
 *
 * <p>A Java migration for the same reason as {@link V13__LoadJavaPractice}: the content is
 * source code, and Flyway would try to interpret parts of it if it went through a plain
 * script. A separate file rather than an edit to the first one, because the first one has
 * already run everywhere and Flyway will not run it again.
 *
 ...
 */
public class V27__LoadJavaCorePracticeExtra extends BaseJavaMigration {

    private static final String CONTENT = "/content/practice/java-core-extra.json";
