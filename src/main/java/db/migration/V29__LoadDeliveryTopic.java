...
/**
 * Adds build, containers and delivery as a study topic: eight sections, their articles and 48 quiz
 * questions.
 *
 * <p>Follows the self-contained layout {@link V6__LoadSqlTopic} established — content under
 * {@code /content/delivery/} rather than in the shared files V2 reads, because V2 has already run
 * everywhere and editing its sources would load a topic on a fresh database while skipping it on
 * an existing one.
 *
 * <p>The material is deliberately written from this repository rather than from general advice:
 * the {@code pom.xml}, the multi-stage {@code Dockerfile} with its AppCDS archive,
 * {@code docker-compose.yml}, {@code render.yaml}, the CI workflow and {@code application.properties}
 * are all on disk, so every number a section quotes can be checked against the file it came from.
 *
 * <p>Career {@code level} is carried on both sections and questions, as {@code V7__levels}
 * requires. The column defaults to {@code MIDDLE}, so a missing level would load silently as
 * middle; {@code DeliveryTopicContentTest} is what holds the content to an explicit value on
 * every row.
 */
public class V29__LoadDeliveryTopic extends BaseJavaMigration {
