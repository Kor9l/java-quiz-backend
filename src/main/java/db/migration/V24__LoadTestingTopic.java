...
/**
 * Adds Testing as a study topic: eight sections, their articles and 48 quiz questions.
 *
 * <p>Follows the self-contained layout {@link V6__LoadSqlTopic} established — content under
 * {@code /content/testing/} rather than in the shared files V2 reads, because V2 has already run
 * everywhere and editing its sources would load a topic on a fresh database while skipping it on
 * an existing one.
 *
 * <p>Like {@link V8__LoadJavaConcurrencyTopic} this one carries the career {@code level} added by
 * {@code V7__levels} on both sections and questions. The column defaults to {@code MIDDLE}, so a
 * missing level would load silently as middle; {@code TestingTopicContentTest} is what holds the
 * content to an explicit value on every row.
 */
public class V24__LoadTestingTopic extends BaseJavaMigration {
