...
/**
 * Adds Observability as a study topic: eight sections, their articles and 48 quiz questions.
 *
 * <p>Follows the self-contained layout {@link V6__LoadSqlTopic} established — content under
 * {@code /content/observability/} rather than in the shared files V2 reads, because V2 has
 ...
 *
 * <p>The topic splits logs, metrics, tracing and error tracking out of the distributed-systems
 * track: the four are asked about separately and carry a section each on their own, and folding
 * them into one section of another topic left no room for cardinality, alerting or profiling.
 *
 * <p>Like every topic since {@code V7__levels} this one carries the career {@code level} on both
 * sections and questions. The column defaults to {@code MIDDLE}, so a missing level would load
 * silently as middle; {@code ObservabilityTopicContentTest} is what holds the content to an
 * explicit value on every row.
 */
public class V31__LoadObservabilityTopic extends BaseJavaMigration {
