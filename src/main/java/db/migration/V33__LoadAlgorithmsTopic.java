...
/**
 * Adds Algorithms and data structures as a study topic: eight sections, their articles and 48
 * quiz questions.
 *
 * <p>Self-contained in the way {@link V6__LoadSqlTopic} established and
 * {@link V8__LoadJavaConcurrencyTopic} repeated — content under {@code /content/algorithms/}
 * rather than in the shared files V2 reads, because V2 has already run everywhere and editing
 * its sources would load a topic on a fresh database while skipping it on an existing one.
 *
 * <p>The career {@code level} is carried on both sections and questions. The column defaults to
 * {@code MIDDLE}, so a missing level would load silently as middle; {@code
 * AlgorithmsTopicContentTest} is what holds the content to an explicit value on every row, and
 * {@link #level(JsonNode)} here is the second line of defence.
 */
public class V33__LoadAlgorithmsTopic extends BaseJavaMigration {
