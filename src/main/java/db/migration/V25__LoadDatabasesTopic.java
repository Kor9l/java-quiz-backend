...
/**
 * Adds Databases as a study topic: eight sections, their articles and 48 quiz questions.
 *
 * <p>Where the {@code sql} topic teaches how to write a query, this one teaches what happens
 * underneath it — isolation, row versions, locks and plans — with PostgreSQL as the reference
 * engine, because that is what this service runs against.
 *
 * <p>Follows the self-contained layout {@link V6__LoadSqlTopic} established and
 * {@link V8__LoadJavaConcurrencyTopic} repeated: content under {@code /content/databases/} rather
 * than in the shared files V2 reads, because V2 has already run everywhere and editing its sources
 * would load a topic on a fresh database while skipping it on an existing one.
 *
 * <p>Sections and questions both carry the career {@code level} added by {@code V7__levels}. The
 * column defaults to {@code MIDDLE}, so a missing level would load silently as middle;
 * {@code DatabasesTopicContentTest} is what holds the content to an explicit value on every row.
 */
public class V25__LoadDatabasesTopic extends BaseJavaMigration {
